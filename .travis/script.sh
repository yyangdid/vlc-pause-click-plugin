...
if [ "$TARGET" = "Linux" ]; then
  ...
  mkdir -p tmp/tmp
  cd tmp/tmp
  wget http://download.videolan.org/pub/videolan/vlc/$VLC_VERSION.0/macosx/vlc-$VLC_VERSION.0.dmg
  ...
  wget http://download.videolan.org/pub/videolan/vlc/$VLC_VERSION.0/win32/vlc-$VLC_VERSION.0-win32.7z -O vlc-$VLC_VERSION.0-win32.7z
  7z x "*.7z" -o* "*/sdk"
